...
byte Medicamento3[4]= {0x8E, 0xEC, 0x5F, 0xD3} ; //código del usuario 2
byte Medicamento4[4]= {0x71, 0x34, 0x0D, 0x08} ; //código del usuario 2
  ...
  if ( mfrc522.PICC_IsNewCardPresent())
  {
    ...
    if ( mfrc522.PICC_ReadCardSerial())
    {
      ...
      }else if(compareArray(ActualUID,Medicamento2,7))
      {
        ...
        lectura_datos();
        si();
      }else if(compareArray(ActualUID,Medicamento4,4))
      {
        Serial.println("Medicamento 2");
        M5.Lcd.setCursor(0, 60);
        M5.Lcd.setTextColor(VERDE);
